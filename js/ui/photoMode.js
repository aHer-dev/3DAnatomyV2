...
// Unten reservierter Streifen auf Mobile = Footprint der beiden Kugeln (S13):
// bottom-Abstand 16 + Kugeldurchmesser 58 = 74px. Damit bleiben Rahmen/Auslöser/
// Vignette über den Kugeln (Safe-Area vernachlässigt).
const MOBILE_ORBS_H = 74;
...
function _updateFrame() {
  ...
  if (mobile) {
    // Sidebar oben → freier Bereich: unter der Sidebar, über den Kugeln
    const sbH      = _sidebar ? (_sidebar.offsetHeight || 60) : 60;
    const toolbarH = MOBILE_ORBS_H;
    margin      = 16;
  ...
  // Auf Mobile: untere Vignette stoppt vor den Kugeln, damit sie sichtbar bleiben
  const toolbarH = mobile ? MOBILE_ORBS_H : 0;
  ...
  if (shutter) {
    ...
    const maxY = mobile ? (vh - MOBILE_ORBS_H - shutterH - 8) : (vh - 12 - shutterH);
    if (sy + shutterH > maxY) {
